...
def addexpense():
    global user_email
    que = "SELECT * FROM expenses where email = ? ORDER BY 'dates' DESC"
    ...
    while dictionary != False:
        exp=(dictionary["ID"],dictionary["DATES"],dictionary["EXPENSENAME"],dictionary["AMOUNT"],dictionary["PAYMODE"],dictionary["CATEGORY"],dictionary["IDX"],dictionary["EMAIL"])
        expense.append(exp)
        dictionary = ibm_db.fetch_assoc(stm)
    i=len(expense)+1
    indexes=str(i)
...
@app.route("/display")
def display():
    query = "SELECT * FROM expenses where email = ? ORDER BY 'dates' DESC"
    stmt = ibm_db.prepare(connection, query)
    ...
    while dictionary != False:
        ...
        rexpense.append(exp)
        dictionary = ibm_db.fetch_assoc(stmt)
    query = "SELECT MONTH(dates) as DATES, SUM(amount) as AMOUNT FROM expenses WHERE email=? AND YEAR(dates)= YEAR(now()) GROUP BY MONTH(dates);"
    stmt = ibm_db.prepare(connection, query)
    ibm_db.bind_param(stmt, 1,session['email'])
    ibm_db.execute(stmt)
    dictionary=ibm_db.fetch_assoc(stmt)
    ...
    query = "SELECT * FROM expenses WHERE email = ? AND YEAR(dates)= YEAR(now());"
    stmt = ibm_db.prepare(connection, query)
    ibm_db.bind_param(stmt, 1,session['email'])
    ibm_db.execute(stmt)
    dictionary=ibm_db.fetch_assoc(stmt)
...
def delete(idx):
    query = "DELETE FROM expenses WHERE email=? and idx=?;"
    stmt = ibm_db.prepare(connection, query)
    ibm_db.bind_param(stmt, 1, session["email"])
...
def edit(id):
    query = "SELECT * FROM expenses WHERE email =? and idx=?;"
    stmt = ibm_db.prepare(connection, query)
    ibm_db.bind_param(stmt, 1, session['email'])
